...
- `best_combo(chain_id, budget, variations=3, refresh=, categories=, promos=, sort_by=)` —
  ...
  - по умолчанию 3 вариации в порядке: **Оптимум** (максимум веса еды в бюджете + вкус,
    напиток оптимизируется внутри подбора) → **Без повторов** (макс. вес, каждый продукт по 1 шт)
    → **Макс. вес** (чистый максимум веса); при `variations>3` — ещё доп. стратегии
